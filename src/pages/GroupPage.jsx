...
export default function GroupPage({ user, onGroupJoined, inline = false }) {
  ...
  // En mode inline, on affiche juste le contenu sans le wrapper plein écran
  const content = (
    <>
      {!inline && <h1 style={styles.title}>🃏 Poker Range</h1>}
      {!inline && <p style={styles.subtitle}>Bienvenue, <strong style={{ color: 'white' }}>{username}</strong></p>}
      {!inline && <p style={styles.hint}>Rejoignez ou créez un groupe pour continuer.</p>}
  ...
  if (inline) {
    return <div style={styles.inlineWrapper}>{content}</div>
  }
...
const styles = {
  ...
  inlineWrapper: { display: 'flex', flexDirection: 'column', gap: '10px', width: '100%' },
